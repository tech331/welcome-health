...
function getPersonDisplayName(fields: Record<string, unknown>): string {
  ...
  if (firstName || lastName) {
    return [fieldToString(firstName), fieldToString(lastName)]
      .filter((part) => part !== "—")
      .join(" ")
      .trim();
  }
  ...
  if (fullName) return fieldToString(fullName);
...
function mapRequestRecord(record: AirtableRecord): RequestRecord {
  const { fields } = record;
  ...
  return {
    ...
    requestor: fieldToString(requestor),
...
export async function getRequests(): Promise<RequestRecord[]> {
  ...
  const records = await fetchAirtableRecords(AIRTABLE_REQUESTS_TABLE_ID);
  return records.map(mapRequestRecord);
}
